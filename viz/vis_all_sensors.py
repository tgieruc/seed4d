...
def get_bbox_images(folder):
    ...
    cam_mesh = np.array(
        ...
        dtype=np.float32,
    )[:, [1, 2, 0]] * -np.array([0.3, 0.2, 0.2])

    cam_mesh = np.concatenate((cam_mesh, np.ones((5, 1))), axis=1)

    np.asarray([frame["transform_matrix"] for frame in data["frames"]])
    ...
    for cam in range(6):
        ...
        for i in range(len(list(bboxes.keys()))):
            ...
            np.asarray(ego_bbox).mean(0)
            rgb_bbox = rgb.copy()
            for _k, bbox in enumerate(bbox_array[:-1]):
        ...
        for img_points in collector:
            for edge in edges_bbox:
                if np.linalg.norm(img_points[edge[0]] - img_points[edge[1]]) < 400:
                    cv2.line(
                        rgb_bbox,
                        (int(img_points[edge[0]][0]), int(img_points[edge[0]][1])),
                        (int(img_points[edge[1]][0]), int(img_points[edge[1]][1])),
                        (255, 0, 0),
                        5,
                    )
...
def plot_sensors_full(args, bbox_images):
    ...
    _fig, axes = plt.subplots(nrows=len(sensor_path_names), ncols=6, figsize=(12, 5))

    indices = {"static": {0, 1, 3, 4}, "dynamic": {0, 1, 3, 4, 5}}
    ...
    for i in tqdm(range(len(sensor_path_names)), desc="Processing full sensors"):
        for j in range(6):
            ...
            if j == 0:
                axes[i, j].text(
                    -1.5, 0.5, f"{labels_row[i]}", fontsize=args.fontsize_full, transform=axes[i, j].transAxes
                )
...
def plot_sensors(args, bbox_images):
    ...
    indices = {"static": {0, 1, 3, 4}, "dynamic": {0, 1, 3, 4, 5, 6}}

    _fig, axes = plt.subplots(nrows=2, ncols=4, figsize=(12, 6))
