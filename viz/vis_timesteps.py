...
def main(args):
    ...
    _fig, axes = plt.subplots(
        nrows=len(args.vehicles),
        ncols=len(args.timesteps),
        figsize=(len(args.timesteps) * 1.8, len(args.vehicles) * 0.8),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--timesteps", nargs="+", help="List of timesteps to plot", default=["4", "8", "15", "16", "27"]
    )
    parser.add_argument(
        "--vehicles",
        nargs="+",
        help="Name vehicles to include. Must be int or ego_vehicles",
        default=["71", "74", "79", "82", "87", "ego_vehicle"],
    )
