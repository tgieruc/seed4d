...
def check_towns(data_path, towns, category):
    ...
    for town in tqdm(towns, desc="Processing towns"):
        ...
        for spawnpoint in tqdm(range(1, spawnpoints + 1), desc=f"Processing spawnpoints in {town}", leave=False):
            ...
            if category == "dynamic":
                data_path_ending = "ClearNoon/vehicle.mini.cooper_s/spawn_point_" + str(spawnpoint)
            if category == "static":
                data_path_ending = "ClearNoon/vehicle.audi.tt/spawn_point_" + str(spawnpoint)
...
def main(args):
    ...
    if incomplete_spawnpoints == []:
        ...
        for incomplete_spawnpoint in incomplete_spawnpoints:
            print(
                f"{incomplete_spawnpoint[0]} spawnpoint {incomplete_spawnpoint[1]} contains {incomplete_spawnpoint[2]} pngs."
            )
